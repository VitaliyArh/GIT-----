...
У файлов созданных в локальном GIT есть статусы:
         ...
         Сохранённый или закоммиченный --- После редактирования и сохранения файла, нужно будет чтобы Git запомнил текущее состояние 
         файла для этого его нужно закоммитить. Теперь ты сможешь вернуться к этой версии файла, если дальше что-то пойдёт не так.
...
17 - git mv ---- ПЕРЕИМЕНОВАНИЕ ФАЙЛА.
                 команда позволяет переименовать уже проиндексированный и закоммиченный файл, и любой другой без лишних действий,
                 то есть файл если он уже закомичен то его не нужно повторно комитить и индексировать.

   ПРИМЕР:
              git mv converter.py data_converter.py
              # переименовали файл converter.py в data_converter.py         

18 - git rm --- УДАЛЕНИЕ ФАЙЛА
                Просто удалить файл не выйдет. Git отметит его как удалённый и продолжит за ним следить.
                Нужно сделать так, чтобы Git перестал отслеживать файл. 
                Для этого есть команда git rm 

   ПРИМЕР:
             git rm data_converter.py   
             # Git удалит файл data_converter.py и перестанет отслеживать его изменения 


Ветвление:

После инициализации репозитория создаётся основная линия разработки — ветка проекта. 
Это действие автоматически запускается командой ------------  git init. 
...
Если вместо master ты видишь main, — всё в порядке. 
Для обозначения основной ветки по умолчанию используется одно из этих имён.


Создать новую ветку

Обычно в master находится стабильная версия проекта. 
Чтобы не испортить её в процессе доработки, создают вторую ветку. 
И уже там экспериментируют: дописывают или изменяют рабочий код

Вторую ветку можно назвать как угодно. Обычно при выборе имени отталкиваются от задачи.

git branch --- команда для создания новой ветки проекта. Нужно только добавить имя ветки.

# создали ветку и назвали её develop
git branch develop 

git checkout название_ветки ---- Чтобы переключится из ветки в ветку 
...
Или одной командой можно сразу и создать и перейти во вновь созданную ветку:

git checkout -b название_ветки 

После создания новой ветки --- новая ветка это абсолютная копия, такая же как и основная ветка master\main.
Поэтому в ней можно кодить и не боятся что оригинал будет испорчен.

Обычная практика в коллективной разработке — каждый раз создавать отдельную ветку для нового функционала. 
Позже код из этой ветки можно будет слить с кодом из основной.
...
ПРИМЕР: 
В ветке master хранится основной класс приложения application.py. 
А в ветке develop для него создаются тесты в новом файле application_test.py.
Ветки нужно слить между собой, чтобы прогресс добавился в основную линию.
Если в файл application.py вносили изменения (менялся) в ветке develop, то при слиянии файл обновится в master.
То есть в ветке master появятся все изменения сделанные в ветке develop.

git merge [название ветки, которую присоединяют] 

перед тем как запустить эту команду нужно перейти в ту ветку в которую нужно слить.
Когда находишься в ветке master - при запуске команды изменения перейдут из develop в master.
...
Чтобы помочь пользователям справиться с этим, Git оставляет подсказки. 
Когда ветки сливаются с конфликтом, в консоли появляется сообщение:

git merge develop
# слили ветку develop в master

CONFLICT (content): Merge conflict in [название файла]
Automatic merge failed; fix conflicts and then commit the result. 

Система объясняет, в каком файле произошли конфликты. 
Нужно их разрешить, а затем сделать коммит с изменениями.

Когда видишь сообщение о конфликтах, открывай проблемный файл в редакторе кода. 
Прямо в коде этого файла Git напишет замечание — в той строке, где текст отличается.
